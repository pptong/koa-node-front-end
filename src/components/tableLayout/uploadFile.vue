<template>
  <div class="uploadFile__component">
    <a-upload name="file" class="file-uploader" :show-upload-list="false" :customRequest="customRequest"
      :disabled="disabled">
...
export default defineComponent({
  ...
  props: {
    ...
    // 上传文件的api接口
    upload: {
      type: Function as PropType<UploadData>,
      required: false
    },
    // 是否禁用
    disabled: {
    ...
    const customRequest = (data: any) => {
      if (props.upload) {
        ...
        props.upload(formdata).then(e => {
          loading.value = false
          context.emit('update:value', e.data.data?.url)
        }).catch(err => {
          loading.value = false
