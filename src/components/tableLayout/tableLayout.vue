<template>
  <a-card class="table__layout">
    <template #title v-if="(selectItem && selectItem.length > 0) || add || $slots.button">
      <div class="table__top" v-if="selectItem && selectItem.length > 0">
        <Form ref="selectForm" name="select" :formItem="selectItem || []" :setData="selectFun" @succeed="selectComplete">
    ...
    <a-table v-bind="tableProps" :rowKey='rowkey' :dataSource="dataSource" :pagination="page === true ? paging : false"
      ...
      <template #operation="item">
        <div class="operation">
          ...
          <slot name="operationMore" :value="item"></slot>
        </div>
      </template>
  ...
  <Modal ref="addModal" title="添加数据" @ok="addSubmit" @cancel="addForm.reset()" v-if="add">
    <Form ref="addForm" name="add" :formItem="addItem ? addItem : formItem" :rules="addRules ? addRules : rules"
      :setData="add" :defaultData="editDefData" @saveSucceed="addComplete" @fail="addModal.loading(false)"
      @saveBefore="createBefore">
      <template v-slot:[formSlot(item)]="{ formData, key }" v-for="item in (addItem ? addItem : formItem)">
        <slot :name="item.slotName" :formData="formData" :key="key" v-if="item.type === 'slot'"></slot>
      </template>
    </Form>
  </Modal>
  <Modal ref="editModal" title="编辑数据" @ok="editSubmit" @cancel="enditCancel" v-if="edit">
    <Form ref="editForm" name="edit" :dataKey="editKey ? editKey : rowkey" :formItem="editItem ? editItem : formItem"
      :rules="editRules ? editRules : rules" :setData="edit" :defaultData="editDefData" @saveSucceed="editComplete"
      @saveBefore="editBefore" @fail="editModal.loading(false)">
...
export default defineComponent({
  ...
  emits: ['editOpen', 'addOpen', 'editSuccess', 'addSuccess', 'editBefore', 'createBefore'],
  props: {
    ...defaultTableProps,
  ...
  setup(props, context) {
    ...
    let currentProps: Record<string, any> = props;


    // 初始化表单配置
    const tableKeys = Object.keys(defaultTableProps)
    const tableProps = computed(() => {
      const dataProps: any = reactive({})
      Object.keys(currentProps).forEach(key => {
        if (tableKeys.includes(key)) {
          dataProps[key] = currentProps[key]
        }
      })
      return dataProps
    })
    ...
    const getData = () => {
      ...
      //alert(JSON.stringify(selectForm.value))
      const params: any = {}
      Object.keys(selectData).forEach(key => {
        if (selectData[key]) {
          params.query[key] = selectData[key]
        }
      })
      ...
      props.get(params).then(e => {
        dataSource.value = e.data.data.data
        const { current, total, pageSize } = e.data.data
    ...
    const createBefore = (data: any) => {
      context.emit('createBefore', data)
    }

    const editBefore = (data: any) => {
      context.emit('editBefore', data)
    }

    // 编辑弹框取消事件
    const enditCancel = (data: any) => {
    ...
    return {
      dataSource, createBefore, editBefore, tableProps, paging, loading, tableChange,
