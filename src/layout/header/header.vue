...
export default defineComponent({
  ...
  computed: {
    ...mapState({
      ...
      avatar: (state: any) => state.auth.avatar,
      routers: (state: any) => {
        const array: any[] = []
        state.auth.routers.forEach((item: any) => { if (!item.hidden) array.push(item) })
        return array
      }
    })
  },
  ...
  setup() {
    ...
    const tabClick = (e: number) => {
      const routers = store.state.auth.routers
      let menuRouter: RouterTable = []
      store.commit('menu/setMenu', routers)
